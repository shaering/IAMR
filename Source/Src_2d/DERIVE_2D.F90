...
#     undef UHIY
      ...
      subroutine FORT_DERBINGHAM (visc,DIMS(visc),nv,dat,DIMS(dat),ncomp,
     &                             lo,hi,domlo,domhi,delta,xlo,time,dt,
     $                             bc,level,grid_no)
      implicit none
c
...
c
      strnrt_fun(ux,vx,uy,vy) = sqrt(ux**2+half*(vx+uy)**2+vy**2)
      bngham_fun(mu,t0,strn,eps) = mu+half*t0*(1-exp(-strn/eps))/strn
      ...
      do j = lo(2), hi(2)
         do i = lo(1), hi(1)
            ...
            visc(i,j,1) =
     &          bngham_fun(visc_coef,yield_stress,strnrt,reg_param)
         end do
      end do
