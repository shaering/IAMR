...
c
      strnrt_fun(ux,vx,uy,vy) = sqrt(ux**2+half*(vx+uy)**2+vy**2)
c
c     ::::: Functions for computing the regularised term in the viscosity
...
c     ::::: Compute Herschel-Bulkley viscosity
...
c
      stress_fun(strnrt) = mu*two**((n+one)/two)*strnrt**n+tau
