                       ...
                       int  ncycle)
{
    ...
    if (theNSPC() != 0 and NavierStokes::initial_iter != true)
    {
      ...
      FillPatchIterator fpi_t(*this,Sstate,nGrow,prev_time,State_Type,Density+2,1);
      MultiFab& temp=fpi_t.get_mf();
...
NavierStokes::predict_velocity (Real  dt)
{
    ...
    //MultiFab rhs(grids,dmap,ncomp,ngrow);    
    rhs.setVal(0.0);
    ...
    MultiFab forcing_term( grids, dmap, AMREX_SPACEDIM, ngrow );
	 ...
	 if (NavierStokes::initial_iter != true) {	    
	    theNSPC()->getDrag(rhs[U_mfi],Umf[U_mfi],Smf[U_mfi],visc_coef[0],ngrow,level);
	 }
     }
...
#endif
    {
        for (MFIter U_mfi(Umf,TilingIfNotGPU()); U_mfi.isValid(); ++U_mfi)
	  {
	    ...
	    */
	    ...
	    //            getForce(forcing_term[U_mfi],gbx,ngrow,Xvel,AMREX_SPACEDIM,prev_time,Ufab,Sfab,rfab,0);
            getForce(forcing_term[U_mfi],gbx,ngrow,Xvel,AMREX_SPACEDIM,prev_time,Ufab,Sfab,0);
                                ...
                                int  lscalar)
{
	...
	floor(Smf);
        ...
        int ncomp = Smf.nComp();
        int ngrow = Smf.nGrow();
        MultiFab rhs(ba,dmap,ncomp,ngrow);
        rhs.setVal(0.0);
   ...
   {
      if (NavierStokes::initial_iter != true) {	         
      theNSPC()->getTemp(rhs[S_mfi],Umf[S_mfi],Smf[S_mfi],visc_coef[0],nGrowF,level);
      }
   }
#endif   
    rhs.SumBoundary(Geom().periodicity()); 	   
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
		...
		*/
		...
		//                getForce(forcing_term[S_mfi],gbx,nGrowF,fscalar,num_scalars,prev_time,Umf[S_mfi],Smf[S_mfi],rhs[S_mfi],0);
                getForce(forcing_term[S_mfi],gbx,nGrowF,fscalar,num_scalars,prev_time,Umf[S_mfi],Smf[S_mfi],0);		
