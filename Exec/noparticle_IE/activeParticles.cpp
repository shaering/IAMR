		...
		if ( i<p_cutoff || gradual_release_flag==0 ) {
		  ...
		  //		    zp = p.m_rdata.arr[AMREX_SPACEDIM+2]; //p.m_rdata.pos[2];
		    ...
		    // upper hole
		       ...
		       // rotate back
		       p.rdata(AMREX_SPACEDIM+0) = ur_prime * cos(theta_p) - ut_prime * sin(theta_p);
                       p.rdata(AMREX_SPACEDIM+2) = ur_prime * sin(theta_p) - ut_prime * cos(theta_p);

		       // back particle out to hole surface		       
                       p.pos(0) = r_top * norm_x;
                       p.pos(2) = r_top * norm_z;		       
		    ...
		    ut_prime = -1.0 * p.rdata(AMREX_SPACEDIM+0) * sin(theta_p) + p.rdata(AMREX_SPACEDIM+2) * cos(theta_p);
		       ...
		       // back particle out to hole surface
                       p.pos(0) = r_bot * norm_x;
                       p.pos(2) = r_bot * norm_z;		       
		    ...
		    // recycle
		       ...
		       p.pos(0) = x_top; //p.m_rdata.pos[0] + 1.0; // fix these hard-codes
                       p.pos(1) = 3.0; //p.m_rdata.pos[1] + 4.48;
                       p.pos(2) = z_top; //p.m_rdata.pos[2];
		  ...
		  p.rdata(AMREX_SPACEDIM+0+3) = 0.0;
		  p.rdata(AMREX_SPACEDIM+1+3) = 0.0;
		  p.rdata(AMREX_SPACEDIM+2+3) = 0.0;		      
